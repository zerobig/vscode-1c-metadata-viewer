...
export class PredefinedDataPanel {
  ...
  public static show(extensionUri: vscode.Uri, metadataName: string, data: PredefinedDataItem[]) {
  ...
  private static _getHtmlForWebview(webview: vscode.Webview, extensionUri: vscode.Uri, data: PredefinedDataItem[]) {
            ...
            <tbody>
              ${!data.length ?
                ...
                `<tr><td colspan="3"><img src="${rootImagePath}" width="16px" height="16px" style="margin-bottom: 3px; vertical-align: middle;" />&nbsp;Элементы</td></tr>
                  ${data.map(i => CreateItem(i, [ folderImagePath, elementImagePath ], 1)).join('')}`
              }
            </tbody>
          </table>
        </body>
...
function CreateItem(item: PredefinedDataItem, imageUri: vscode.Uri[], level: number): string {
  ...
  ${(() => {
    if (item.ChildItems && item.ChildItems.Item) {
      return item.ChildItems.Item.map(i => CreateItem(i, imageUri, level + 1)).join('');
    }
